...
test_that("alphagenome_get_cage extracts cage data from response", {

  mock_response <- list(
    cage = list(
      values    = matrix(rnorm(100), nrow = 10),
      metadata  = data.frame(name = "test_cage", strand = "+")
    )
  )

  result <- alphagenome_get_cage(mock_response)

  expect_type(result, "list")
  expect_true(length(result) > 0)
  expect_true("cage" %in% names(mock_response))
