...
test_that("alphagenome_query validates region length <= 1 Mb", {
  skip_on_cran()
  expect_error(
    alphagenome_query(access_token = "dummy", genomic_region = "chr17:1-2000000", requested_outputs = "RNA_SEQ"),
...
test_that("alphagenome_query validates requested_outputs", {
  skip_on_cran()
  expect_error(
    alphagenome_query(access_token = "dummy", genomic_region = "chr17:42560601-43609177", requested_outputs = "INVALID_TYPE"),
    "requested_outputs"  # or whatever your actual stopifnot message is
  )
})
