...
test_that("alphagenome_get_procap extracts procap data from response", {

  mock_response <- list(
    procap = list(
      values    = matrix(rnorm(100), nrow = 10),
      metadata  = data.frame(name = "test_procap", strand = "+")
    )
  )

  result <- alphagenome_get_procap(mock_response)

  expect_type(result, "list")
  expect_true(length(result) > 0)
  expect_true("procap" %in% names(mock_response))
