...
test_that("alphagenome_get_splice_sites extracts splice_sites data from response", {

  mock_response <- list(
    splice_sites = list(
      values    = matrix(rnorm(100), nrow = 10),
      metadata  = data.frame(name = "test_splice_sites", strand = "+")
    )
  )

  result <- alphagenome_get_splice_sites(mock_response)

  expect_type(result, "list")
  expect_true(length(result) > 0)
  expect_true("splice_sites" %in% names(mock_response))
