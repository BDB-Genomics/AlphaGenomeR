...
test_that("alphagenome_get_splice_usage extracts splice_site_usage data from response", {
  ...
  mock_response <- list(
    splice_site_usage = list(
      values    = matrix(rnorm(100), nrow = 10),
      metadata  = data.frame(name = "test_splice_site_usage", strand = "+")
    )
  )

  result <- alphagenome_get_splice_usage(mock_response)

  expect_type(result, "list")
  expect_true(length(result) > 0)
  expect_true("splice_site_usage" %in% names(mock_response))
