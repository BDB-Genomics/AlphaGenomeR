...
message("Querying AlphaGenome API: ", region_str)
...
plot_track_labeled <- function(data, title, color = "steelblue") {
  ...
  if (!is_valid) {
    ...
    v <- data$values
    # Robustly get first column or vector
    if (is.matrix(v) || is.array(v)) {
       d <- dim(v)
       if (length(d) >= 2 && d[2] > 0) {
          track_to_plot <- as.numeric(v[, 1])
       } else {
          track_to_plot <- as.numeric(as.vector(v))
       }
    } else {
  ...
  df <- data.frame(
    Coord = seq(start_coord, end_coord, length.out = length(track_to_plot)),
    Signal = track_to_plot
  )
  
  ggplot(df, aes(x = Coord, y = Signal)) +
    geom_area(fill = color, alpha = 0.75) +
    geom_line(color = color, linewidth = 0.5) +
    scale_x_continuous(labels = function(x) paste0(round(x/1e6, 3), "Mb")) +
    labs(title = paste(title, "|", track_name), x = "Genomic Position", y = "Predicted Intensity") +
    theme_genomic()
}

save_pro("man/figures/gallery/res_atac.png", plot_track_labeled(alphagenome_get_atac(predictions), "Chromatin Accessibility (ATAC)", "firebrick3"))
save_pro("man/figures/gallery/res_cage.png", plot_track_labeled(alphagenome_get_cage(predictions), "Transcription Initiation (CAGE)", "forestgreen"))
save_pro("man/figures/gallery/res_dnase.png", plot_track_labeled(alphagenome_get_dnase(predictions), "Regulatory Elements (DNase)", "darkorange"))
save_pro("man/figures/gallery/res_rna.png", plot_track_labeled(alphagenome_get_rna_seq(predictions), "Gene Expression (RNA-seq)", "royalblue4"))
save_pro("man/figures/gallery/res_histone.png", plot_track_labeled(alphagenome_get_chip_histone(predictions), "Histone Modification", "darkorchid4"))
save_pro("man/figures/gallery/res_tf.png", plot_track_labeled(alphagenome_get_chip_tf(predictions), "TF Binding", "indianred4"))
save_pro("man/figures/gallery/res_splice_sites.png", plot_track_labeled(alphagenome_get_splice_sites(predictions), "Splice Site Probability", "cyan4"))
...
save_pro("man/figures/gallery/res_procap.png", plot_track_labeled(alphagenome_get_procap(predictions), "Nascent Transcription", "slateblue4"))
...
if (!is.null(sj) && !is.null(sj$junctions)) {
  tryCatch({
    ...
    p_sj <- ggplot(df_sj) +
      ...
      scale_linewidth_continuous(range = c(0.8, 4)) +
      scale_x_continuous(labels = function(x) paste0(round(x/1e6, 3), "Mb")) +
      labs(title = "Figure 3 | Predicted Splice Junction Strength", x = "Genomic Position", y = "") +
      theme_genomic() + theme(axis.text.y = element_blank())
    save_pro("man/figures/gallery/res_junctions.png", p_sj, w=15, h=6)
  }, error = function(e) message("  SJ Error: ", e$message))
}
...
if (!is.null(cm) && !is.null(cm$values)) {
  tryCatch({
    ...
    p_cm <- ggplot(df_cm, aes(X, Y, fill = Prob)) +
      ...
      labs(title = "Figure 4 | 3D Genome Contact Map", x = "Bin X", y = "Bin Y") +
  ...
  }, error = function(e) message("  CM Error: ", e$message))
}

# --- ATLAS (Final attempt with simple logic) ---
message("Building Final Atlas...")
p1 <- plot_track_labeled(alphagenome_get_rna_seq(predictions), "RNA-seq", "royalblue4")
p2 <- plot_track_labeled(alphagenome_get_atac(predictions), "ATAC", "firebrick3")
p3 <- plot_track_labeled(alphagenome_get_cage(predictions), "CAGE", "forestgreen")

# Combine using gridExtra for stability
atlas_grid <- grid.arrange(p1, p2, p3, ncol = 1)
save_pro("man/figures/modality_atlas.png", atlas_grid, w = 15, h = 12)
